(function() {
	...
	var tmp_http = {
		...
		cbFunction: function(info) {
			...
			if(info) {
				var respCode = getHTTPStatus(info[0]);
				if(respCode == "2xx")
					http.test.successMsg = "HTTP tests passed.";
				else
					http.test.successMsg = "URL/server problem.";
			}
			http.test.cbExecuted = true;
			// update the tables
			globalUpdate("http", info);
		},
		execChildren: true,
		successMsg: "",
		failureMsg: "Checking for locally configured proxies.",
		shortDesc: "HTTP entity retrival",
