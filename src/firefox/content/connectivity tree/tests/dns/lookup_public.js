(function() {
	...
	var tmp_lookup = {
		name: "Lookup via public resolver",
		test: function() {
			//log(0, lookup.test, JSON.stringify(lookup.test.input));
			lookup_test(domainName, "8.8.8.8", "udp", tmp_lookup.cbFunction);
		...
		cbFunction: function(info) {
			...
			// update the tables
			globalUpdate("lookup_public", info);
		},
		...
		execChildren: true,
		successMsg: "Configured resolver is broken.",
		failureMsg: "Lookup via a public resolver failed. Resolving a standard name.",
		shortDesc: "IP address lookup for the domain, via a public resolver",
		longDesc: "This test tries to retrieve the 'A' record for the domain via a public resolver."
	};
