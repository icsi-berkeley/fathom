(function() {
	...
	function lookup_test(name, srv, proto, callback) {
		...
		var data = fathom.proto.dns.query(dns, object.domain, 1, 1, 0x0110);
		fathom.proto.dns.sendRecv(dns, srv, object.DEST_PORT, data, recordSend, recordReceive);
	}

	var tmp_lookup = {
		name: "Lookup host w/ CD",
		test: function() {
		...
		execChildren: true,
		successMsg: "Resolved with CD bit set, so it is a DNSSEC problem.",
		failureMsg: "CD Lookup failed. Resolving with a public resolver.",
		shortDesc: "IP address lookup for the domain, with the CD option",
		longDesc: "This test tries to retrieve the 'A' record for the domain via the configured resolver by setting the CD option."
	};
