(function() {
	...
	var tmp_lookup = {
		name: "Lookup standard name",
		test: function() {
			//log(0, lookup.test, JSON.stringify(lookup.test.input));
			lookup_test(standardName, DNSserver, "udp", tmp_lookup.cbFunction);
		...
		cbFunction: function(info) {
			...
			log(0, lookup.test, tmp_lookup.toHTML([standardName, info]));
			// update the tables
			globalUpdate("lookup_standard", info);
		},
		...
		execChildren: true,
		successMsg: "'A' record for www.google.com exists.",
		failureMsg: "DNS problem. Check path for UDP/53.",
		shortDesc: "IP address lookup for a popular domain",
		longDesc: "This test tries to retrieve the 'A' record for a standard domain via the configured resolver."
	};
